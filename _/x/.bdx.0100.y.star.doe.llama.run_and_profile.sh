#!/bin/bash
# .bdx.0100.y.star.doe.llama.run_and_profile.sh
# 20230910_225422
# 20230914_234326
# 20230926_205146
# 20230927_105758
...
#_star_doe_conf__pkg_install_sh=".star.doe.pkg.install.sh"; ### come from $_star_doe_conf
_speedscope_viewer="speedscope";
...
function _star_doe_testcase_prepare()
{
	...
	_star_prof_pyspy_out_json=".star.prof.${_xprmnt_uuid}.pyspy_out.json"; # generated by py-spy
	...
	_star_doe_conf__prof_out_save_path_pyspy="${_tmp_star_d}/${_star_prof_pyspy_out_json}";
	...
	_star_prof_asys_script_pyspy=".star.prof.${_xprmnt_uuid}.asys_script.pyspy.sh"; # 
	#
	...
	export STAR_DOE_CONF__PROF_OUT_SAVE_PATH_PYSPY="${_star_doe_conf__prof_out_save_path_pyspy}"
}
...
function main_exec_runtype_profile_w_pyspy()
{
	echo "### { ========================================================================================== ";
	echo "###	function main_exec_runtype_profile_w_pyspy(): start";
	_main_python_file=".star.doe.main.testcase.pyspy.py";   ### _main_python_file="example_text_completion.py";
		### .star.doe.main.testcase.pyspy.py@    --> example_text_completion.star.pyspy.py
	_pyspy_output_filename="$_star_prof_pyspy_out_json";
	#_pyspy_output_format="speedscope"; ### { "flamegraph", "raw", "speedscope" } ### come from $_star_doe_conf
	#_pyspy_sampling_rate="100"; ### 100 by default ### come from $_star_doe_conf
	py-spy record --full-filenames -o ${_tmp_star_d}/${_pyspy_output_filename} -f $_pyspy_output_format -r $_pyspy_sampling_rate -s -t -n -- \
	torchrun --nproc_per_node $_nproc_per_node $_main_python_file --ckpt_dir $_ckpt_dir  --tokenizer_path tokenizer.model --max_seq_len $_max_seq_len --max_batch_size $_max_batch_size ;
	echo "###	${_main_python_file}: completed";
	( cd $_tmp_star_d;
	cat > $_star_prof_asys_script_pyspy << EOF_ASYS_SCRIPT
#!/bin/bash
$_speedscope_viewer $_star_prof_pyspy_out_json &
EOF_ASYS_SCRIPT
	chmod 755 $_star_prof_asys_script_pyspy;
	);
	echo "###	Please run ${_star_prof_asys_script_pyspy} to see the profiling result";
	echo "###	function main_exec_runtype_profile_w_pyspy(): end";
	echo "### } ========================================================================================== ";
}
...
function _star_doe_testcase_exec()
{
	...
	case $_star_doe_conf__runtype in
		...
		"profile")
			...
			;;
		"pyspy")
			2>&1 main_exec_runtype_profile_w_pyspy | tee -a $_star_doe_testcase_log;
			;;
		*)
			2>&1 main_exec_runtype_dummy | tee -a $_star_doe_testcase_log;
			;;
...
_list__max_seq_len="64"; ### max_seq_len should be larger than the input sequence lenght # please make sure to assert ( max_prompt_len <= params.max_seq_len ) # llama/generation.py", line 141, in generate()
_list__max_gen_len="64";
_list__max_batch_size="1";
_list__xprmnt_cpu_model="CORE-I7-1270P";
_pyspy_output_format="speedscope"; ### { "flamegraph", "raw", "speedscope" }
_pyspy_sampling_rate="100"; ### 100 by default
